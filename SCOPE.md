...
- `Player.OnPlay` on real play/resume, `Player.OnPause` on real pause, `Player.OnStop` on completion and every player exit path.
...
- Send the final stop before player Back/Activity finish and suppress duplicate stops after normal completion.
- Note for testing: Scrob persists Continue Watching only after a stop above its 5% progress floor (and below 90%).
