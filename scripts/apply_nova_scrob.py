...
import android.app.Activity;import android.graphics.Typeface;import android.os.Bundle;import android.os.Handler;import android.os.Looper;import android.view.Gravity;import android.widget.Button;import android.widget.LinearLayout;import android.widget.TextView;import com.archos.mediacenter.utils.scrob.Scrob;import org.json.JSONObject;
public class ScrobAuthActivity extends Activity {
 private final Handler main=new Handler(Looper.getMainLooper()); private TextView status,code; private Button authorize,disconnect; private volatile boolean cancelled=false; private String verify;
 @Override protected void onCreate(Bundle b){super.onCreate(b);setTitle("Scrob authorization");int pad=(int)(24*getResources().getDisplayMetrics().density);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(pad,pad,pad,pad);root.setGravity(Gravity.CENTER_HORIZONTAL);TextView title=new TextView(this);title.setText("NOVA Scrob");title.setTextSize(24);title.setTypeface(Typeface.DEFAULT,Typeface.BOLD);root.addView(title);status=new TextView(this);status.setPadding(0,pad,0,pad/2);root.addView(status);code=new TextView(this);code.setTextSize(22);code.setTypeface(Typeface.MONOSPACE,Typeface.BOLD);code.setGravity(Gravity.CENTER);root.addView(code);authorize=new Button(this);authorize.setText("Authorize with Scrob");authorize.setOnClickListener(v->startAuth());root.addView(authorize,new LinearLayout.LayoutParams(-1,-2));disconnect=new Button(this);disconnect.setText("Forget authorization");disconnect.setOnClickListener(v->{cancelled=true;Scrob.clearAuthorization(this);code.setText("");refresh();});root.addView(disconnect,new LinearLayout.LayoutParams(-1,-2));TextView hint=new TextView(this);hint.setPadding(0,pad,0,0);hint.setText("Use your phone or computer to open the displayed Scrob link and enter the short code. NOVA Scrob never stores your Scrob password.");root.addView(hint);setContentView(root);refresh();}
 private void refresh(){String u=Scrob.baseUrl(this);String server=u.isEmpty()?"Scrob URL not set":"Server: "+u;status.setText(server+"\nStatus: "+Scrob.status(this));disconnect.setEnabled(Scrob.hasAuthorization(this));}
 private void startAuth(){if(Scrob.baseUrl(this).isEmpty()){status.setText("Set your Scrob URL in Settings first.");return;}cancelled=false;authorize.setEnabled(false);status.setText("Contacting Scrob…");code.setText("");new Thread(()->{try{Scrob.HttpResult s=Scrob.requestDeviceCode(this);if(!s.ok()){fail("Could not start authorization (HTTP "+s.code+")");return;}JSONObject j=s.body;String device=j.optString("device_code","");String user=j.optString("user_code","");verify=j.optString("verification_uri",Scrob.baseUrl(this)+"/link");int interval=Math.max(2,j.optInt("interval",5)),expires=Math.max(60,j.optInt("expires_in",900));if(device.isEmpty()||user.isEmpty()){fail("Unexpected response from Scrob");return;}main.post(()->{code.setText("Code: "+user+"\n\nOpen on your phone/computer:\n"+verify);status.setText("Waiting for approval…");});long deadline=System.currentTimeMillis()+expires*1000L;int poll=interval;while(!cancelled&&System.currentTimeMillis()<deadline){try{Thread.sleep(poll*1000L);}catch(InterruptedException ignored){}if(cancelled)return;Scrob.HttpResult t=Scrob.exchangeDeviceCode(this,device);if(t.ok()&&Scrob.hasAuthorization(this)){main.post(()->{authorize.setEnabled(true);code.setText("Authorized ✓\nSaved on this device.");refresh();});return;}String e=t.body.optString("error","");if("authorization_pending".equals(e))continue;if("slow_down".equals(e)){poll+=5;continue;}if("access_denied".equals(e)){fail("Authorization declined");return;}if("expired_token".equals(e)||"invalid_grant".equals(e)){fail("Code expired. Try again.");return;}}if(!cancelled)fail("Authorization timed out. Try again.");}catch(Exception e){fail("Could not reach Scrob: "+e.getMessage());}},"ScrobAuth").start();}
 private void fail(String s){main.post(()->{authorize.setEnabled(true);status.setText(s);});}
...
if 'com.archos.mediacenter.video.scrob.ScrobAuthActivity' not in m:
    ...
    m=m.replace('</application>',f'        <activity android:name="com.archos.mediacenter.video.scrob.ScrobAuthActivity" android:exported="true">\n            <intent-filter>\n                <action android:name="org.courville.novascrob.action.SCROB_AUTH" />\n                <category android:name="android.intent.category.DEFAULT" />\n            </intent-filter>\n        </activity>\n    </application>',1)
...
print('NOVA Scrob v0.1.1 patch applied. appId='+APP_ID)
